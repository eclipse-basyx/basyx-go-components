...
func TestIntegration(t *testing.T) {
	...
	for i, config := range configs {
		...
		t.Run(name, func(t *testing.T) {
			...
			if config.Action == "DELETE_ALL_SUBMODEL_DESCRIPTORS" {
				// Fetch current descriptors
				body, err := makeRequest(TestConfig{Method: "GET", Endpoint: "http://127.0.0.1:6005/submodel-descriptors", ExpectedStatus: 200}, i+1)
				...
				for _, item := range list.Result {
					...
					_, err := makeRequest(TestConfig{Method: "DELETE", Endpoint: fmt.Sprintf("http://127.0.0.1:6005/submodel-descriptors/%s", enc), ExpectedStatus: 204}, i+1)
					require.NoError(t, err)
				}
				return
			}
