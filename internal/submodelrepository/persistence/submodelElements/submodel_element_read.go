...
func getSubmodelElementsByDatabaseID(ctx context.Context, db dbQueryer, submodelDatabaseID int64, limit *int, cursor string, level string) ([]types.ISubmodelElement, string, error) {
	...
	parsedRows, readRowsErr := readSubmodelElementRowsByRootIDs(db, submodelDatabaseID, rootIDs, includeChildren, isGetSubmodelElements)
	if readRowsErr != nil {
		return nil, "", readRowsErr
	}
...
func readSubmodelElementRowsByPath(db dbQueryer, submodelDatabaseID int64, idShortOrPath string, includeChildren bool) ([]loadedSMERow, error) {
	dialect := goqu.Dialect("postgres")
...
func readSubmodelElementRowsByRootIDs(db dbQueryer, submodelDatabaseID int64, rootIDs []int64, includeChildren bool, isGetSubmodelElements bool) ([]loadedSMERow, error) {
	if len(rootIDs) == 0 {
