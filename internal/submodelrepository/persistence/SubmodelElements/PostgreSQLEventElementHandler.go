...
func (p PostgreSQLEventElementHandler) Create(submodelId string, submodelElement gen.SubmodelElement) (int, error) {
	...
	// Start a database transaction at the EventElement level
	tx, err := p.db.Begin()
	if err != nil {
		return 0, err
	}
...
func (p PostgreSQLEventElementHandler) CreateNested(submodelId string, idShortPath string, submodelElement gen.SubmodelElement) (int, error) {
