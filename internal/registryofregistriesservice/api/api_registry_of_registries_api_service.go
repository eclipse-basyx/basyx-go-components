...
func (s *RegistryOfRegistriesAPIAPIService) GetAllRegistryDescriptors(ctx context.Context, limit int32, cursor string, company string, endpointInterface string) (model.ImplResponse, error) {
	var internalCursor string
	if strings.TrimSpace(cursor) != "" {
		dec, decErr := common.DecodeString(cursor)
		if decErr != nil {
			log.Printf("📍 [%s] Error in GetAllRegistryDescriptors: decode cursor=%q limit=%d company=%q endpointInterface=%q: %v", componentName, cursor, limit, company, endpointInterface, decErr)
	...
	aasds, nextCursor, err := s.registryOfRegistriesBackend.ListRegistryDescriptors(ctx, limit, internalCursor, company, endpointInterface)
	if err != nil {
		log.Printf("📍 [%s] Error in GetAllRegistryDescriptors: list failed (limit=%d cursor=%q company=%q endpointInterface=%q): %v", componentName, limit, internalCursor, company, endpointInterface, err)
		switch {
