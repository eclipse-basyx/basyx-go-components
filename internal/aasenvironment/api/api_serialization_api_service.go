...
func (s *AASEnvironmentSerializationAPIAPIService) resolveSerializationSupplementaryParts(
	...
	for _, fileLocation := range fileLocations {
		...
		if _, seen := seenSupplementaries[resolvedReference]; seen {
			log.Printf("[WARN] AASENV-SERIALIZESUPPL-SKIPDUPLICATE duplicate supplementary URI '%s' for submodel '%s' at path '%s'", resolvedReference, fileLocation.SubmodelID, fileLocation.IDShortPath)
			continue
		}
...
func (s *AASEnvironmentSerializationAPIAPIService) resolveSerializationThumbnailParts(
	...
	for _, aasID := range resolvedAASIDs {
		...
		if _, seen := seenThumbnailURIs[resolvedThumbnailURI]; seen {
			log.Printf("[WARN] AASENV-SERIALIZETHUMB-SKIPDUPLICATE skipping duplicate thumbnail URI '%s' for AAS '%s'", resolvedThumbnailURI, aasID)
			continue
		}
...
// resolveThumbnailPartURI builds the package URI for a thumbnail filename.
func resolveThumbnailPartURI(thumbnailFileName, thumbnailContentType string) (*url.URL, error) {
	return url.Parse("/" + resolveThumbnailPartFileName(thumbnailFileName, thumbnailContentType))
}

// resolveThumbnailPartFileName normalizes a thumbnail filename and appends an
// inferred extension when missing.
func resolveThumbnailPartFileName(fileName, contentType string) string {
	normalizedFileName := normalizeAASXPartFileName(fileName)
	if normalizedFileName == "" {
		normalizedFileName = "thumbnail"
	}

	if normalizeFileExtension(filepath.Ext(normalizedFileName)) != "" {
		return normalizedFileName
	}

	thumbnailExtension := supplementaryFileExtension(normalizedFileName, contentType)
	if thumbnailExtension == "" {
		return normalizedFileName
	}

	return normalizedFileName + thumbnailExtension
}
