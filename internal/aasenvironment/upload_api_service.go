...
func (s *uploadAPIService) HandleUpload(ctx context.Context, fileName string, contentType string, file *os.File) (commonmodel.ImplResponse, error) {
	...
	// #nosec G703 -- file path is generated by commonmodel.ReadFormFileToTempFile via os.CreateTemp.
	...
	format, resolvedContentType, err := detectUploadFormat(normalizedFileName, contentType, rawContent)
	if err != nil {
	...
	switch format {
	case "aasx":
		return s.handleAASXUpload(ctx, normalizedFileName, resolvedContentType, rawContent)
	case "json":
		return s.handleJSONUpload(ctx, normalizedFileName, resolvedContentType, rawContent)
	case "xml":
		return s.handleXMLUpload(ctx, normalizedFileName, resolvedContentType, rawContent)
	default:
...
func detectUploadFormat(fileName string, contentType string, rawContent []byte) (string, string, error) {
	...
	default:
		return detectUploadFormatFromContent(rawContent)
	}
}
...
func detectUploadFormatFromContent(rawContent []byte) (string, string, error) {
	trimmed := bytes.TrimSpace(rawContent)
	if len(trimmed) == 0 {
		return "", "", fmt.Errorf("unable to determine upload format from empty payload")
	}

	if len(rawContent) >= 4 && rawContent[0] == 0x50 && rawContent[1] == 0x4b {
		return "aasx", "application/aasx+xml", nil
	}
