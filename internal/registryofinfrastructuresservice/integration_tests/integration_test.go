...
func TestIntegration(t *testing.T) {
	...
	for i, config := range configs {
		...
		t.Run(fmt.Sprintf("Step_(%s)_%d_%s_%s", context, i+1, config.Method, config.Endpoint), func(t *testing.T) {
			...
			if config.Action == "DELETE_ALL_INFRASTRUCTURE_DESCRIPTORS" {
				// Fetch current descriptors
				body, err := makeRequest(TestConfig{Method: "GET", Endpoint: "http://127.0.0.1:5004/infrastructure-descriptors", ExpectedStatus: 200}, i+1)
				...
				for _, item := range list.Result {
					...
					_, err := makeRequest(TestConfig{Method: "DELETE", Endpoint: fmt.Sprintf("http://127.0.0.1:5004/infrastructure-descriptors/%s", enc), ExpectedStatus: 204}, i+1)
					require.NoError(t, err)
				}
				return
			}
