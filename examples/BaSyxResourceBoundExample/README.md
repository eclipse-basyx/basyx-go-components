...
1. Replace `https://issuer.example`, the audience, and subject IDs in `config.yaml`, `trustlist.json`, `resource-policies.json`, and `fallback.json` with your verified OIDC identities. The owner needs a token for the configured audience. `bridge-auditor` demonstrates object-based ABAC fallback.
...
Initial resource policies are imported only when the configured scope is first initialized. Restarting does not overwrite access changes. Manage subsequent policy changes through `/$access/policy`. Stop the example with `docker compose down`.
