...
| Low impact | The AASX File Server now supports durable asynchronous package uploads through `POST /packages-async`, with status polling and retained result endpoints that work across service replicas and restarts. | [#609](https://github.com/eclipse-basyx/basyx-go-components/pull/609) | Async upload creation requires the same create authorization as synchronous uploads; status and result access require read authorization and are isolated to the submitting owner. |
