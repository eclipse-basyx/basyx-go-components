...
| Low impact | AAS Repository, Submodel Repository, and registry `getAll` endpoints now use bounded, set-based database reads whose query count is independent of the requested page size. Pagination, ordering, representations, and filters remain compatible. | [#606](https://github.com/eclipse-basyx/basyx-go-components/pull/606) | Authorization behavior is unchanged; existing ABAC filters and fragment masks remain enforced. |
