...
- Audit metadata is populated when `history.auditIdentityMode` is `minimal` or `extended` and request/OIDC/ABAC metadata is available. BaSyx preserves valid `X-Request-ID` and `X-Correlation-ID` values from clients or trusted ingress and generates defaults when they are missing or invalid. These identifiers are stored in traceable HTTP history rows but are not treated as authenticated identity data. Anonymous/local requests remain valid with empty actor identity fields.
...
BaSyx preserves a valid, specific content type declared by the current upload instead of replacing it with byte-sniffing results. MIME resolution uses the first valid non-generic current declaration, a specific byte-sniffed type, the filename extension, a valid generic current declaration, and finally `application/octet-stream`, in that order. Multipart attachment uploads use the file part's content type as the current declaration. AASX imports order current declarations as the File SME content type followed by the supplementary package-part content type. Existing File SME metadata is consulted only when those current declarations are missing, invalid, or limited to the `application/octet-stream` placeholder. This prevents stale metadata from overriding a current `text/plain`, XML, or ZIP declaration while preserving structured text formats such as IFC (`application/x-step`), CSV (`text/csv`), and SVG (`image/svg+xml`) that byte sniffers can otherwise classify as plain text or XML.
...
The MIME-resolution change does not rewrite existing File metadata that was previously stored with a generic or incorrect content type. Reset or re-import affected preconfigured environments, or replace the affected File metadata and attachment, to apply the declared content type.
